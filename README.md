...
`ginx` 负责请求绑定、默认值、校验、成功/失败响应、拦截器、非 JSON 响应、SSE、JSON Lines / NDJSON，以及可选的 OpenAPI 代码生成。
...
1. 创建 `Req` 并应用 `default` tag
2. 按字段和请求 `Content-Type` 绑定多个来源
3. 汇总执行 `binding` 校验
4. 调用业务 Handler
5. 写出成功或失败响应
...
ginx.Handle(router, methods, path, handler, opts...)
ginx.SSE(router, path, handler, opts...)
ginx.JSONLines(router, method, path, handler, opts...)
```

`router` 可以是 `*gin.Engine`、`*gin.RouterGroup`、`engine.Wrap(...)` 或 `engine.Group(...)` 返回的 `*ginx.Router`。

`Any` 会注册 GET、POST、PUT、PATCH、DELETE、HEAD、OPTIONS；`Handle` 接受自定义 method 列表。SSE 固定使用 GET，JSON Lines 则显式接收 method。
...
JSON body 只会在 `Content-Type` 为 `application/json`、带参数的 JSON 类型或 `+json` 媒体类型时绑定；form-urlencoded 和 multipart body 也按各自的 `Content-Type` 选择。path、query、header、cookie 仍可与其中一种 body 来源组合。

`WithStrictJSONBody(true)` 的“严格”特指拒绝一个 body 中连续出现多个 JSON 值；它不会拒绝 schema 之外的未知 JSON 字段。
...
ginx.POST(r, "/jobs", CreateJob, ginx.SuccessStatus(http.StatusAccepted))
ginx.POST(r, "/legacy", Login, ginx.AlwaysOK())
```

`SuccessStatus` 固定成功状态且只接受 2xx；非法值会在路由注册时 panic。对 JSON 成功响应，状态优先级是 `AlwaysOK()`、`SuccessStatus(...)`、`WithSuccessHandler` 返回值、默认 200。204 和 HEAD 不写响应 body。`StringRsp` / `DataRsp` 也支持固定成功状态；`FileRsp` 的 200/206 由文件传输决定，`RedirectRsp` 使用自身的重定向状态。
...
其中 `WithSuccessHandler` 只作用于启用 data wrap 的 JSON 成功响应；`NoDataWrap()` 和实现了 `ginx.Response` 的非 JSON 响应不会经过它。
...
JSON Lines / NDJSON 使用显式 HTTP method 注册：

```go
ginx.JSONLines(r, http.MethodPost, "/logs", func(ctx context.Context, req *TailReq, send ginx.JSONLinesSender) error {
	for _, item := range loadLogs(ctx, req) {
...
每次 `send` 会写出一个紧凑 JSON 值、追加换行并立即 flush，响应类型为 `application/x-ndjson`。首条记录前的错误仍走标准 JSON 错误响应；流开始后的错误只记录到 `gin.Context.Errors` 并结束连接，不会向数据流追加错误封装。
