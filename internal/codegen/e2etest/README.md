...
Generation can also be run without the Tier-A/Tier-B tests:

```sh
scripts/generate-codegen-e2e.sh
```

The OpenAPI 3.0 and 3.1 generated fixture files are ignored by Git. On a
fresh checkout, run one of the scripts above before commands that load all
packages, such as `go test ./...`, `go vet ./...`, or `govulncheck ./...`.
