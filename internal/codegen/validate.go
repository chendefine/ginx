...
func validateResponseVariantTypeNames(ops []OperationDef, types []TypeDef) error {
	seen := make(map[string]string)
	for _, td := range types {
		if name, kind := typeDefName(td); name != "" {
			seen[name] = kind
		}
	}
	for _, op := range ops {
		if len(op.ResponseVariants) == 0 {
			continue
		}
		name := op.RspTypeName
		source := op.Method + " " + op.Path + " response variants"
		if previous, ok := seen[name]; ok {
			return fmt.Errorf("generated type conflict: %s and %s both generate %s; rename the schema or adjust operationId", previous, source, name)
		}
		seen[name] = source
	}
	return nil
