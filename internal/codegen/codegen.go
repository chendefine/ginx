...
func GenerateMulti(cfg Config) (*GenerateResult, error) {
	...
	// OpenAPI 3.2 introduces `in: querystring`. kin-openapi v0.142.0 rejects it
	...
	if err := validateResponseVariantTypeNames(ops, allTypes); err != nil {
		return nil, err
	}
...
// to `in: query`. kin-openapi v0.142.0 rejects querystring during Validate,
