...
func TestSuccessStatusWrappedAndRaw(t *testing.T) {
	for _, tc := range []struct {
		name   string
		status int
		opts   []RouteOption
	}{
		{name: "wrapped-created", status: http.StatusCreated, opts: []RouteOption{SuccessStatus(http.StatusCreated)}},
		{name: "raw-accepted", status: http.StatusAccepted, opts: []RouteOption{NoDataWrap(), SuccessStatus(http.StatusAccepted)}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			r := gin.New()
			POST(r, "/status", func(context.Context, *simpleReq) (*simpleRsp, error) {
				return &simpleRsp{Message: "done"}, nil
			}, tc.opts...)
			w := httptest.NewRecorder()
			r.ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/status", nil))
			if w.Code != tc.status {
				t.Fatalf("status=%d, want=%d body=%s", w.Code, tc.status, w.Body.String())
			}
			if !strings.Contains(w.Body.String(), `"message":"done"`) {
...
func TestSuccessStatusPrecedence(t *testing.T) {
	e := New(WithSuccessHandler(func(context.Context, any) (int, any) {
		return http.StatusAccepted, gin.H{"source": "handler"}
	}))
	r := gin.New()
	POST(e.Wrap(r), "/fixed", func(context.Context, *simpleReq) (*simpleRsp, error) {
		return &simpleRsp{}, nil
	}, SuccessStatus(http.StatusCreated))
	POST(e.Wrap(r), "/always", func(context.Context, *simpleReq) (*simpleRsp, error) {
		return &simpleRsp{}, nil
	}, SuccessStatus(http.StatusCreated), AlwaysOK())

	for path, want := range map[string]int{"/fixed": http.StatusCreated, "/always": http.StatusOK} {
		w := httptest.NewRecorder()
		r.ServeHTTP(w, httptest.NewRequest(http.MethodPost, path, nil))
		if w.Code != want {
			t.Fatalf("%s status=%d, want=%d", path, w.Code, want)
		}
		if !strings.Contains(w.Body.String(), `"source":"handler"`) {
			t.Fatalf("%s body=%s", path, w.Body.String())
		}
	}
}

func TestNoBodySuccessSkipsRenderer(t *testing.T) {
	for _, method := range []string{http.MethodDelete, http.MethodHead} {
		t.Run(method, func(t *testing.T) {
			rendered := false
			successHandled := false
			e := New(
				WithSuccessHandler(func(context.Context, any) (int, any) {
					successHandled = true
					return http.StatusOK, nil
				}),
				WithJSONRenderer(func(c *gin.Context, status int, body any) {
					rendered = true
					c.JSON(status, body)
				}),
			)
			r := gin.New()
			Handle(e.Wrap(r), []string{method}, "/empty", func(context.Context, *simpleReq) (*simpleRsp, error) {
				return &simpleRsp{Message: "must-not-render"}, nil
			}, SuccessStatus(http.StatusNoContent))
			w := httptest.NewRecorder()
			r.ServeHTTP(w, httptest.NewRequest(method, "/empty", nil))
			if w.Code != http.StatusNoContent || w.Body.Len() != 0 {
				t.Fatalf("status=%d body=%q", w.Code, w.Body.String())
			}
			if rendered {
				t.Fatal("JSON renderer should not run for a no-body response")
			}
			if successHandled {
				t.Fatal("success handler should not run for a fixed 204 response")
			}
		})
	}
}

func TestFixedStatusAppliesToStringAndDataResponses(t *testing.T) {
	r := gin.New()
	POST(r, "/string", func(context.Context, *simpleReq) (*StringRsp, error) {
		return StringResponse(http.StatusOK, "created"), nil
	}, SuccessStatus(http.StatusCreated))
	POST(r, "/data", func(context.Context, *simpleReq) (*DataRsp, error) {
		return DataResponse(http.StatusOK, "application/octet-stream", []byte("accepted")), nil
	}, SuccessStatus(http.StatusAccepted))
	DELETE(r, "/empty", func(context.Context, *simpleReq) (*StringRsp, error) {
		return StringResponse(http.StatusOK, "must-not-render"), nil
	}, SuccessStatus(http.StatusNoContent))
	...
	for _, tc := range []struct {
		...
		{http.MethodPost, "/string", http.StatusCreated, "created"},
		{http.MethodPost, "/data", http.StatusAccepted, "accepted"},
		{http.MethodDelete, "/empty", http.StatusNoContent, ""},
	} {
		w := httptest.NewRecorder()
...
func TestHeadSuccessSuppressesBodyAtStatusOK(t *testing.T) {
	r := gin.New()
	HEAD(r, "/head", func(context.Context, *simpleReq) (*simpleRsp, error) {
		return &simpleRsp{Message: "must-not-render"}, nil
	})
	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodHead, "/head", nil))
	if w.Code != http.StatusOK || w.Body.Len() != 0 {
		t.Fatalf("status=%d body=%q", w.Code, w.Body.String())
	}
}

func TestSuccessStatusRejectsNon2xxAtRegistration(t *testing.T) {
	defer func() {
		if recover() == nil {
			t.Fatal("expected invalid SuccessStatus to panic")
		}
	}()
	r := gin.New()
	GET(r, "/bad", func(context.Context, *simpleReq) (*simpleRsp, error) { return nil, nil }, SuccessStatus(http.StatusFound))
}

type testResponseVariant struct {
	status int
	body   any
}

func (r *testResponseVariant) GinxResponseVariant() (int, any) { return r.status, r.body }

func TestResponseVariantUsesDeclaredStatusAndWrapping(t *testing.T) {
	r := gin.New()
	POST(r, "/variant", func(context.Context, *simpleReq) (*testResponseVariant, error) {
		return &testResponseVariant{status: http.StatusAccepted, body: &simpleRsp{Message: "queued"}}, nil
	}, RouteInterceptor(func(ctx context.Context, req any, next func() (any, error)) (any, error) {
		return next()
	}))
	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/variant", nil))
	if w.Code != http.StatusAccepted || !strings.Contains(w.Body.String(), `"message":"queued"`) {
		t.Fatalf("status=%d body=%s", w.Code, w.Body.String())
	}
}

func TestResponseVariantNoBodyAndInvalidStatus(t *testing.T) {
	r := gin.New()
	GET(r, "/empty-variant", func(context.Context, *simpleReq) (*testResponseVariant, error) {
		return &testResponseVariant{status: http.StatusNoContent}, nil
	})
	GET(r, "/invalid-variant", func(context.Context, *simpleReq) (*testResponseVariant, error) {
		return &testResponseVariant{}, nil
	})
	GET(r, "/redirect-variant", func(context.Context, *simpleReq) (*testResponseVariant, error) {
		return &testResponseVariant{status: http.StatusFound}, nil
	})

	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/empty-variant", nil))
	if w.Code != http.StatusNoContent || w.Body.Len() != 0 {
		t.Fatalf("empty status=%d body=%q", w.Code, w.Body.String())
	}
	w = httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/invalid-variant", nil))
	if w.Code != http.StatusInternalServerError || !strings.Contains(w.Body.String(), "invalid status 0") {
		t.Fatalf("invalid status=%d body=%s", w.Code, w.Body.String())
	}
	w = httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/redirect-variant", nil))
	if w.Code != http.StatusFound || w.Body.Len() != 0 {
		t.Fatalf("redirect status=%d body=%q", w.Code, w.Body.String())
	}
}
