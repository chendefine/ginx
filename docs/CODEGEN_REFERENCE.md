...
- schema 的 `description` 会生成对应 Go struct、enum 或 alias 的文档注释。
- schema property 的 `description` 会生成对应 Go struct 字段的文档注释，包括组件类型、inline JSON body、form-urlencoded 和 multipart 字段。
- parameter 自身的 `description` 会生成请求字段注释；未填写时回退到 parameter schema 的 `description`。
- operation 的 `summary` 和 `description` 会生成 server/client 接口方法的文档注释。
- 多行 description 会逐行转换为合法的 `//` 注释，并保留段落空行。
