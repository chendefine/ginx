...
这是 `ginx` 运行时 API 的完整参考手册，保留详细说明、边界和速查表。首次接入建议先阅读 [README.md](../README.md) 的主线用法，再按需回到本文查细节。
...
### 8.5 `SuccessStatus(code)`

为路由固定 2xx 成功状态；非 2xx 会在注册时 panic。普通 JSON 响应的状态优先级为：

1. `AlwaysOK()`
2. `SuccessStatus(code)`
3. `WithSuccessHandler` 返回的状态
4. 200

固定状态仍保留 `WithSuccessHandler` 生成的 body。204 与 HEAD 只写状态，不调用成功包装器后的 JSON renderer，也不写 body。`StringRsp`/`DataRsp` 会采用固定状态；`RedirectRsp` 使用构造器中的 3xx；`FileRsp` 保留 `http.ServeFile` 的 Range/206 行为。
...
codegen 的复杂响应判别容器会实现 `ResponseVariant`。runtime 在普通 `Response` 之前识别它，并按其 2xx/3xx 状态写出 JSON 或无 body 响应；无效状态按内部错误处理。
...
如果你希望 AI Agent 快速掌握本库用法，仓库提供了两个入口：

- [AGENTS.md](../AGENTS.md)：通用 AI Agent 入口，说明应读取哪些文档和采用什么工作流
- [skills/ginx-http-backend/SKILL.md](../skills/ginx-http-backend/SKILL.md)：可复制到 Codex skill registry 的精简技能包，可作为 `$ginx-http-backend` 使用

这个 skill 方案是可行的：它把 handler 签名、Engine 配置快照、安全默认、OpenAPI codegen 工作流、响应/错误模式和验收命令压缩成 AI 可直接执行的操作规则，避免每次让 Agent 从完整 README 重新归纳。
...
- `ResponseVariant` — codegen 复杂 operation 的状态/body 判别接口
- `UnexpectedStatusError` — 客户端实际状态不在契约集合中的错误
- `Interceptor` — 拦截器签名
...
- `ParseResponse(statusCode, body, result)`
- `ValidateResponseStatus(status, expected...)`
