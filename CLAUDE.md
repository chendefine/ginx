...
- OpenAPI 3.2: `openapi: "3.2.0"` loads/validates; kin-openapi v0.142.0 preserves `MediaType.itemSchema`; `in: querystring` is normalized to `query`. The remaining unsupported structural fields (`QUERY` method, `additionalOperations`, structured Tags) are rejected clearly by validation — see `TestE2E_OAI32_UnsupportedStructuralFieldsRejected`.
